...
static int header_handler(CURL *handle, curl_infotype type, char *data, size_t size, void *userdata) 
{
	if(type == CURLINFO_HEADER_OUT) {
		if(strncmp(data, "HEAD", 4) == 0) INC_HEADS;
		if(strncmp(data, "GET", 3) == 0) INC_GETS;
		// S3 pricing is the same for all below 
		if(strncmp(data, "PUT", 3) == 0) INC_PUTS;
		if(strncmp(data, "COPY", 4) == 0) INC_COPYS;
		if(strncmp(data, "POST", 4) == 0) INC_POSTS;
		if(strncmp(data, "LIST", 4) == 0) INC_LISTS;

		// Unused params squelch
		(void)(handle);
		(void)(userdata);
		(void)(size);
	}
...
#define return_status(status)                                           \
	...
	curl_easy_setopt(request->curl, CURLOPT_DEBUGFUNCTION, header_handler);
	int t = 10;
	curl_easy_setopt(request->curl, CURLOPT_DEBUGDATA, &t); 
...
TOC("Before cURL");
		...
		if ((code = curl_easy_getinfo(request->curl, CURLINFO_SIZE_DOWNLOAD, &down_sz)) != CURLE_OK) {
            fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
        }

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_SIZE_UPLOAD, &up_sz)) != CURLE_OK) {
            fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
        }

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_NAMELOOKUP_TIME, &lu_time)) != CURLE_OK) {
			fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
		}

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_CONNECT_TIME, &con_time)) != CURLE_OK) {
			fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
		}

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_APPCONNECT_TIME, &appcon_time)) != CURLE_OK) {
			fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
		}

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_PRETRANSFER_TIME, &pre_time)) != CURLE_OK) {
            fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
        }

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_STARTTRANSFER_TIME, &st_time)) != CURLE_OK) {
            fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
        }

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_TOTAL_TIME, &tot_time)) != CURLE_OK) {
            fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
        }

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_REDIRECT_TIME, &redir_time)) != CURLE_OK) {
			fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
		}

		if ((code = curl_easy_getinfo(request->curl, CURLINFO_REDIRECT_COUNT, &redir_count)) != CURLE_OK) {
			fprintf(stderr, "ERROR: curl_easy_getinfo :: %d", code);
		}

		TIME_IT_INSERT(down_sz, "Download size");
		TIME_IT_INSERT(up_sz, "Upload size");
		TIME_IT_INSERT(lu_time, "Lookup time");
		TIME_IT_INSERT(con_time, "Connect time");
		TIME_IT_INSERT(appcon_time, "App connect time (SSL)");
		TIME_IT_INSERT(pre_time, "Pretransfer time");
		TIME_IT_INSERT(st_time, "Start time");
		TIME_IT_INSERT(tot_time, "Total time");
		TIME_IT_INSERT(redir_time, "Redirect time");
TIC;
