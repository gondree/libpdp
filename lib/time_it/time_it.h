...
#define MAX_TIMERS 16384
#define MAX_CSV_CELL 4096

/* One timing result, from TIC to TOC */
...
/* All timing results in data,
 * and other bookkeeping variables */
...
extern timer* time_it_ptr;

#ifdef TIME_IT
#define TIC { time_it_start_timer(time_it_ptr, __LINE__); }

#define TOC(comment) { time_it_stop_timer(time_it_ptr, __LINE__, __func__, __FILE__, comment); }

#define TIME_IT_INSERT(time, comment) { time_it_insert_time(time_it_ptr, time, __LINE__, __LINE__, __func__, __FILE__, comment); } 

#define PRINT_TIME { \
	struct time_it_timer_data cur = time_it_ptr->data[(time_it_ptr->idx)-1]; \
...
#define PRINT_ALL_TIMES { \
	int __i; \
	struct time_it_timer_data cur = {0}; \
	printf("\n\n====== TIMES ======\n"); \
	for(__i = 0; __i < time_it_ptr->idx; __i++){ \
		cur = time_it_ptr->data[__i]; \
		printf("%s:%d-%d in %s :: %lf :: %s\n", \
				cur.file, cur.st_line, cur.en_line, cur.func, cur.time, cur.comment); \
	} \
	printf("===================\n"); \
};

#define TIME_IT_ADD_META(meta) { if(time_it_ptr) time_it_ptr->meta = meta; }
