/**
 * @file
 * Implementation of the MR-PDP module for flat-file storage.
 *
 ...
 **/
int mrpdp_write_tags_to_file(const pdp_ctx_t *ctx, const pdp_mrpdp_tagdata_t* t)
{
 ...
 * @return 0 on success, non-zero on error
 **/
int mrpdp_get_tag_file(const pdp_ctx_t* ctx, unsigned int index, 
                      void *buf, unsigned int *len)
{
    pdp_mrpdp_tag_t **tag_ptr = (pdp_mrpdp_tag_t **) buf;
    ...
    *tag_ptr = tag;
    ...
    data_len = mrpdp_serialized_tag_size(ctx);
    ...
    if (mrpdp_deserialize_tag(ctx, tag, data, data_len) != 0)
        goto cleanup;
                        ...
                        void *b, unsigned int *len)
{
    ...
    return pdp_get_block_file(ctx, i, ctx->mrpdp_param->block_size, 
                              (unsigned char *) b, len);
}
