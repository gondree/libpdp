 ...
 **/
int apdp_write_tags_to_file(const pdp_ctx_t *ctx, const pdp_apdp_tagdata_t* t)
{
    ...
    if ((apdp_serialize_tags(ctx, t, &buf, &len)) != 0) goto cleanup;
    do {        
 ...
 * @return 0 on success, non-zero on error
 **/
int apdp_get_tag_file(const pdp_ctx_t* ctx, unsigned int index, 
                      void *buf, unsigned int *len)
{
    pdp_apdp_tag_t **tag_ptr = (pdp_apdp_tag_t **) buf;
    ...
    *tag_ptr = tag;
    ...
    data_len = apdp_serialized_tag_size(ctx);
    ...
    if (apdp_deserialize_tag(ctx, tag, data, data_len) != 0)
        goto cleanup;
                        ...
                        void *b, unsigned int *len)
{
    ...
    return pdp_get_block_file(ctx, i, ctx->apdp_param->block_size, 
                              (unsigned char *) b, len);
}
