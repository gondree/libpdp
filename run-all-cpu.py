...
times_dir = "./data/all-times-cpu"

cpor = "./src/cpor/cpor-cpu"
macpdp = "./src/macpdp/por-cpu"
sepdp = "./src/sepdp/sepdp-cpu"
apdp = "./src/apdp/pdp-cpu"

methodologies = [
	dict(path=cpor, args="", bucket='cpor'),
	dict(path=macpdp, args="", bucket='macpdp'),
	dict(path=sepdp, args="", bucket='sepdp'),
	dict(path=apdp, args="", bucket='apdp')
]
...
for sz in file_sizes:
	for method in methodologies:
		for i in range(exp_count):
			...
			cmd = ["cd "+mlist[0]+" ; "+
					"./"+mlist[1]+" "+method["args"]+" ../../"+test_fname_pat+sz+" "+num_loops_cpu+" ; "+
					"cd ../../"+test_file_dir+"; rm -f *.tag *.tok *.t"]
